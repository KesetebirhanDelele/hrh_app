        ...
        for item in fa.get("operational_barriers", []):
            ...
            if not has_kw:
                ...
                if not any(kw in stmt for kw in ob_stmt_keywords):
                    ob_fails.append(f"  OB {domain_id}/{fa_id}/{item['item_id']}: {snippets[0][:80] if snippets else 'NO SNIPPET'}")
        ...
        for item in fa.get("consequences_impacts", []):
            ...
            if not has_kw:
                ...
                if not (stmt_has_kw and snip_broad):
                    ci_fails.append(f"  CI {domain_id}/{fa_id}/{item['item_id']}: {snippets[0][:80] if snippets else 'NO SNIPPET'}")

total = len(ob_fails) + len(ci_fails)
print(f"{total} potential failures:")
for f in ob_fails + ci_fails:
    print(f)
